...
bool Round0BCMessage::ToProtoObject(safeheron::proto::multi_party_ecdsa::Li24::sign::Round0BCMessage &message) const {
    bool ok = true;

    safeheron::proto::CurvePoint point;
    ok = Gk_.ToProtoObject(point);
    if (!ok) return false;
    message.mutable_gk()->CopyFrom(point);
    return true;
}

bool Round0BCMessage::FromProtoObject(const safeheron::proto::multi_party_ecdsa::Li24::sign::Round0BCMessage &message) {
    bool ok = true;

    ok = Gk_.FromProtoObject(message.gk());
    ok = ok && !Gk_.IsInfinity();
    if (!ok) return false;
...
typedef safeheron::proto::multi_party_ecdsa::Li24::sign::Round0BCMessage ProtoObject;
