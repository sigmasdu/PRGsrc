...
void testCoSign_t_n(std::vector<std::string> &sign_key_base64,int threshold,int n_parties) {
    ...
    for(size_t i = 0;i<t;i++)
    {
        ok = safeheron::multi_party_ecdsa::Li24::trim_sign_key(sign_key_base64[i], sign_key_base64[i], participant_id_arr);
    ...
    for(size_t i = 0;i<t;i++)
    {
        ok = Context::CreateContext(party_context[i], sign_key_base64[i], m);
        EXPECT_TRUE(ok);
    }
