...
import (
	...
	"github.com/taurusgroup/multi-party-sig/protocols/Li24"
	"io"
	"io/ioutil"
	"log"
	"net"
...
func processCmd(uid party.ID, tcp *test.NetworkP2P, ids party.IDSlice, threshold int, group curve.Secp256k1) {
	...
	for cmd := range tcp.CMD() {
		...
		//	preSign, err = Li24PreSign(cfg, signers, tcp, pl)
		//	if err != nil {
		//		log.Print("Error", err)
		//	}
		...
		//	err = Li24PreSignOnline(cfg, preSign, []byte(messageToSign), tcp, pl)
		//	if err != nil {
		//		log.Print("Error", err)
		//	}
		...
		case "sign":
			...
			err = Li24Sign1(bipCfg, []byte(messageToSign), signers, tcp, pl)
			if err != nil {
				log.Print("Error", err)
			}
		...
		case "refresh":
			...
			cfgNew, err := Li24Refresh1(cfgOld, tcp, pl)
			if err != nil {
		...
		case "gen":
			cfg, err := Li24Keygen1(uid, ids, threshold, tcp, pl)
			if err != nil {
...
func genAddress(cfgOld *Li24.Config, path string) {
...
func writeConfig(cfg *Li24.Config, f string) {
	cfb, _ := cfg.MarshalBinary()
	ioutil.WriteFile(f, cfb, 0644)
}

func loadConfig(f string, group curve.Secp256k1) (*Li24.Config, error) {
...
func Li24Keygen1(id party.ID, ids party.IDSlice, threshold int, n test.INetwork, pl *pool.Pool) (*Li24.Config, error) {
	h, err := protocol.NewMultiHandler(Li24.Keygen(curve.Secp256k1{}, id, ids, threshold, pl), nil)
	if err != nil {
		return nil, err
	}
	...
	return r.(*Li24.Config), nil
}

func Li24Refresh1(c *Li24.Config, n test.INetwork, pl *pool.Pool) (*Li24.Config, error) {
	hRefresh, err := protocol.NewMultiHandler(Li24.Refresh(c, pl), nil)
	if err != nil {
		return nil, err
	}
...
func Li24Sign1(c *Li24.Config, m []byte, signers party.IDSlice, n test.INetwork, pl *pool.Pool) error {
	h, err := protocol.NewMultiHandler(Li24.Sign(c, signers, m, pl), nil)
	if err != nil {
		return err
	}
	...
	if !signature.Verify(c.PublicPoint(), m) {
		return errors.New("failed to verify Li24 signature")
	}
...
//func Li24PreSign(c *Li24.Config, signers party.IDSlice, n test.INetwork, pl *pool.Pool) (*ecdsa.PreSignature, error) {
//	h, err := protocol.NewMultiHandler(Li24.Presign(c, signers, pl), nil)
//	if err != nil {
//		return nil, err
//	}
//
...
//		return nil, errors.New("failed to verify Li24 presignature")
//	}
//	return preSignature, nil
//}

//func Li24PreSignOnline(c *Li24.Config, preSignature *ecdsa.PreSignature, m []byte, n test.INetwork, pl *pool.Pool) error {
//	h, err := protocol.NewMultiHandler(Li24.PresignOnline(c, preSignature, m, pl), nil)
//	if err != nil {
//		return err
//	}
//	test.HandlerLoop(c.ID, h, n)
