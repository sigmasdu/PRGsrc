...
func (p *MPCDotLib) Sign(p2p []string, id, threshold int, keyShare []byte, path string, messageToSign []byte) ([]byte, []byte, byte, error) {
	...
	sig, err := Li24Sign1(bipCfg, messageToSign, signers, network, pl)
	if err != nil {
		return nil, nil, 0, err
	}
...
func (p *MPCDotLib) RefreshKey(p2p []string, id, threshold int, keyShare []byte) ([]byte, error) {
	...
	cfgNew, err := Li24Refresh1(cfgOld, network, pl)
	if err != nil {
...
func (p *MPCDotLib) GenKey(p2p []string, id, threshold int) ([]byte, error) {
	...
	cfg, err := Li24Keygen1(uid, ids, threshold, network, pl)
	if err != nil {
		return nil, err
	}
...
func Li24Sign1(c *Li24.Config, m []byte, signers party.IDSlice, n test.INetwork, pl *pool.Pool) (*ecdsa.Signature, error) {
	...
	if !signature.Verify(c.PublicPoint(), m) {
		return nil, errors.New("failed to verify Li24 signature")
	}
...
func loadConfig(cfb []byte, group curve.Secp256k1) (*Li24.Config, error) {
